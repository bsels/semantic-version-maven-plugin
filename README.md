...
| `versioning.git`                   | `Git`                | `NO_GIT`                                                          | Defines the git operation mode:<br/>• `NO_GIT`: no git operations will be performed<br/>• `STAGING`: stage all changed files (added to the git index)<br/>• `STASH`: added changed files to the git staging (deprecated since {DEPRECATION_VERSION}: use STAGING)<br/>• `COMMIT`: commit all changed files with the configured commit message |
...
| `versioning.update.scripts`          | `String`             | `-`                                                       | Script paths to execute per updated module, separated by the OS path separator (`:` on Unix-like systems, `;` on Windows). Each script runs in the module directory with `CURRENT_VERSION`, `NEW_VERSION`, `DRY_RUN` (`true` or `false`), `GIT_STASH` (`true` or `false`. Deprecated since {DEPRECATION_VERSION}: use `GIT_STAGING`), `GIT_STAGING` (`true` or `false`), `PROJECT_PATH` (absolute path to module directory) and `EXECUTION_DATE` (yyyy-MM-dd) environment variables. For instance, to automate deprecation version management: `-Dversioning.update.scripts=./update-deprecation-version.sh` (adjusting the paths to the project root). |
...
This script automates the replacement of the `{DEPRECATION_VERSION}` placeholder in the codebase.
