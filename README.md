...
| `versioning.git`                   | `Git`                | `NO_GIT`                                                          | Defines the git operation mode:<br/>• `NO_GIT`: no git operations will be performed<br/>• `STAGING`: stage all changed files (added to the git index)<br/>• `STASH`: added changed files to the git staging (deprecated: use STAGING)<br/>• `COMMIT`: commit all changed files with the configured commit message             |
